#!/usr/bin/env bash
# run-bursts.sh -- which kind of work on another core makes the slow window? Synthetic
...
# 0.75 + 0.3 i s + U(0, 0.1) s into the round, each marked in the trace just before:
#   U  three "change" writes to tj-thermal's uevent file (the control: the real thing)
#   N  nothing (the null control)
#   C  3 ms of compute in registers (no memory traffic)
#   M  3 ms of 1 MB memcpy's through a 64 MB buffer touched before the round (memory
#      traffic)
#   T  3 ms of mmap 64 KB / touch / munmap (each munmap's TLB flush is broadcast to every
#      core on arm64)
#   S  3 ms of getppid() (kernel entry and exit, no memory-map change)
# The injector allocates nothing, forks nothing and maps nothing during the round except
# by T itself. Nothing on the board is changed. Core 5 shares its L3 with the probe's
# core 4, not with QEMU's cores 0-2, so M's traffic reaches QEMU through the memory
# system beyond the L3, and the probe's side through the shared L3 as well.
#
...
# (frames into tap-qnx, every thermal zone read, the markers; tjphase_trace.py). SSH
...
#
# THE RULE, fixed here before any run (bursts_report.py applies it):
#   - alignment as in run-uevent.sh; each timed exchange falls in one class, the first
#     that matches: tj (-0.5..+6 ms of a tj-thermal read), then each injection kind in
#     the order U N C M T S (-0.5..+8 ms of its marker), other (-0.5..+6 ms of another
#     zone's read), out;
#   - a class's EXCESS is the median round trip of its exchanges minus the median of
#     the out class, pooled over the aligned rounds.
#
# THE PREDICTION, written and committed before any run of this harness, smoke runs
# included. It is not to be amended. H: the window is interference through the memory
# system: memory traffic or broadcast TLB maintenance on another core slows the guest;
# compute and plain syscalls do not.
#   P1 compute is not it: C's excess is below +10 us. REFUTED at +10 or more.
#   P2 memory traffic makes a window: M's excess is >= +20 us. PARTIAL at +10 to +20.
#      REFUTED below +10.
#   P3 broadcast TLB maintenance makes a window: T's excess is >= +20 us. PARTIAL at
#      +10 to +20. REFUTED below +10.
#   P4 plain syscalls are not it: S's excess is below +10 us. REFUTED at +10 or more.
...
#   M5 each burst ran its length: C, M, T and S have median durations of 2.5-3.6 ms, and
#      every burst did at least one iteration -> P1-P4
#   M6 no SSH login was accepted during the rounds -> all
#   M7 the null windows are quiet: N's excess is within +-10 us, so the injector's own
#      wake-ups and markers do not make a window -> P1-P4
# Scored only at k = 24.
#
...
REPORT="${REPORT:-$here/bursts_report.py}"
...
m_prepare_out "${OUT:-}" "$HOME/bursts-out"
...
INTERVAL_MS=2 m_write_stamp "$OUT/stamp.json" \
	...
	"\"port\": $D_PORT" \
	"\"injector\": \"burst_inject.c (sha256 $(_sha "$INJ_SRC")) on core $CORE_AUX, zone $ZONE_TYPE ($(basename "$TZ")): per round one each of U N C M T S, 3 ms bursts, slots 0.75 + 0.3 i s + U(0, 0.1) s, seed SEED*100 + round, SEED $SEED\"" \
...
# ---------------------------------------------------------------- the run
say "k=$K rounds of t2ms, n=$N, warmup=$WARMUP, one each of U N C M T S in each -> $OUT"
RUN_T0=$(date +%s)
: > "$OUT/thermal.log"
IPID=""
for r in $(seq 1 "$K"); do
	...
	taskset -c "$CORE_AUX" sudo -n "$INJ" --zone "$TZ" --marker "$TRACE/trace_marker" \
		--log "$OUT/inj-t2ms_r$r.jsonl" --seed $((SEED * 100 + r)) --kinds UNCMTS \
		--start 0.75 --step 0.3 --jitter 0.1 --burst-ms 3 &
	IPID=$!
...
say "which kind of work makes the window?"
