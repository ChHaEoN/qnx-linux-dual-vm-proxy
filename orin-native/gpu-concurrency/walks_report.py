...
-0.5..+6 ms of a traced tj-thermal read), then each injection kind K in U N P F A (T0
...
def main(argv):
    ...
    print("  P1  udevd's sysfs path walk makes a window: P %+.1f us (slow from +%.0f) -> %s"
          % (ex["P"], SLOW, verdict(score_slow(ex["P"]), scored, k, ok, allm)))
    print("  P2  the same walk on tmpfs does not: F %+.1f us (quiet below +%.0f) -> %s"
          % (ex["F"], QUIET, verdict(score_quiet(ex["F"]), scored, k, ok, allm)))
    print("  P3  sysfs attribute reads make a window: A %+.1f us (slow from +%.0f) -> %s"
          % (ex["A"], SLOW, verdict(score_slow(ex["A"]), scored, k, ok, allm)))
    print("  P4  the uevents still make it (the control): U %+.1f us (slow from +%.0f) -> %s"
