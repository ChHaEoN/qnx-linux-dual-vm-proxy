#!/usr/bin/env python3
"""caches_report.py OUT N WARMUP -- run-caches.sh's test: do cache-missing memory walks, or
cross-core wake-ups, make the slow window? (Phase 3b / A6, 2026-09-24), by the rule its
header fixed before any run. The same rule as bursts_report.py, with the kinds U N Q R L W.
...
-0.5..+6 ms of a traced tj-thermal read), then each injection kind K in U N Q R L W (T0
...
def main(argv):
    ...
    print("  P1  a 2 MB random walk on core 3 (QEMU's L3) makes a window: Q %+.1f us (slow from +%.0f) -> %s"
          % (ex["Q"], SLOW, verdict(score_slow(ex["Q"]), scored, k, ok, allm)))
    print("  P2  the same walk on core 5 (the probe's L3) makes one too: R %+.1f us (slow from +%.0f) -> %s"
          % (ex["R"], SLOW, verdict(score_slow(ex["R"]), scored, k, ok, allm)))
    print("  P3  a walk that fits a core's L2 does not: L %+.1f us (quiet below +%.0f) -> %s"
          % (ex["L"], QUIET, verdict(score_quiet(ex["L"]), scored, k, ok, allm)))
    print("  P4  cross-core wake-ups do not: W %+.1f us (quiet below +%.0f) -> %s"
          % (ex["W"], QUIET, verdict(score_quiet(ex["W"]), scored, k, ok, allm)))
