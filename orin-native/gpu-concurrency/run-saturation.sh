...
# gpu_cpu6 must show GR3D at 50% or more; every window's trace holds at least
# half the samples its length calls for; and after the run, every arm file --
...
run_arm() {   # $1 tag  $2 round
	local tag="$1" r="$2" t="$1_r$2" thr cores gpu prio pre="" prc
	...
	m_probe "$OUT" "$t" "$GUEST" "$PORT" "$pre"; prc=$?
