#!/usr/bin/env bash
# run-confine.sh -- confine the host's userspace to cores 3 and 5, away from QEMU's cores
# (0-2) and the probe's (4): how much of the uevent window, and of the tail, goes?
...
#             (the graphical session; the guest and the probe run in the harness's scope,
#             which stays 0-5, and QEMU keeps its own 0-2 pinning);
#   open      the same units set to AllowedCPUs=0-5.
# Checked on the board before this was written: the setting applies at once (udevd's
# Cpus_allowed_list becomes 3,5); an EMPTY AllowedCPUs= does NOT undo it on systemd 249
# (udevd stays 3,5), so open sets 0-5 explicitly; and no userspace task outside the root
# cgroup had an affinity narrower than 0-5, so restoring 0-5 loses nothing. After every
# switch the harness waits 1 s and reads the allowed CPUs of udevd's main process, PID 1,
# gnome-shell (when running) and every QEMU thread (confine.log). Kernel threads stay where
# they are. On any exit every unit is set back to 0-5, the runtime drop-ins are removed and
# systemd reloaded, and udevd is checked at 0-5.
...
# default (two vCPUs, halt_poll_ns 500000, 2 ms). The light trace of run-uevent.sh (frames
# into tap-qnx, every thermal zone read, the markers; tjphase_trace.py). SSH logins accepted
# during the rounds are counted from the journal (logins.txt): the run is watched from the
# one session that started it.
#
# THE RULE, fixed here before any run (confine_report.py applies it):
...
#   - an arm's p50 and p99.9 are over every timed exchange of its aligned rounds, pooled.
#
...
#   P4 the typical exchange does not change: |confined p50 - open p50| <= 2 us. REFUTED
#      beyond.
# THE CHECKS (a prediction resting on a failed one prints VOID):
#   M1 >= 90% of rounds align -> all
#   M2 >= 90% of U injections raised the uevent seqnum by >= 3 -> P1
#   M3 every round's allowed CPUs fit its arm: udevd, PID 1 and gnome-shell 3,5 when
#      confined and 0-5 when open -> all
#   M4 >= 50 U and >= 40 tj exchanges in each arm -> P1 P2
#   M5 no SSH login was accepted during the rounds -> all
#   M6 the null windows are quiet in both arms: N's excess within +-10 us -> P1 P2
...
REPORT="${REPORT:-$here/confine_report.py}"
...
m_prepare_out "${OUT:-}" "$HOME/confine-out"
...
INTERVAL_MS=2 m_write_stamp "$OUT/stamp.json" \
	...
	"\"port\": $D_PORT" \
	"\"injector\": \"burst_inject.c (sha256 $(_sha "$INJ_SRC")) on core $CORE_AUX, zone $ZONE_TYPE ($(basename "$TZ")): per round three U and one N, slots 0.75 + 0.4 i s + U(0, 0.1) s; units $UNITS set to AllowedCPUs=$CONF_CORES (confined) or 0-5 (open) per round in the pattern ${CPAT[*]}, seed SEED*100 + round, SEED $SEED\"" \
...
# ---------------------------------------------------------------- the run
say "k=$K rounds of t2ms, n=$N, warmup=$WARMUP, three U and one N in each, userspace confined/open ($UNITS) -> $OUT"
RUN_T0=$(date +%s)
: > "$OUT/thermal.log"
IPID=""
for r in $(seq 1 "$K"); do
...
say "confined to 3 and 5: what goes?"
