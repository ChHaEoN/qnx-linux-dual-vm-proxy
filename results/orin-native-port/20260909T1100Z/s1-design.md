...
| C7 | The TCG leg's config gate would reject `cmdline`, `initrd`, `logger` and `set` (`scripts/qhv/g2.conf.allow:25-33`), yet its header says `post_start.custom` runs the validator (`:5`), which the committed snippet does not do (RT reader check; §Appendix A) | **S1 uses its own allow-list** (`s1/s1-conf.allow`) and gate, run on the PC at build time and in both generators. TCR-CFG-001's allow-list governs the QNX guest's configuration and is not edited (D13) | VERIFIED |
...
- **`scripts/qhv/g2.conf.allow:5`:** says `post_start.custom` runs the validator before qvm; the committed snippet does not (C7).
