...
**2026-09-17 (dated append): the third wording is the one that applies.** Under OD9 the owner reversed freeze item 2 to **QNX plus Linux**, so item 3 became applicable and branch 1 of the rule above is the branch taken. B5 then ran on the board and passed (§6.10's append; `<rec>/B5/parse-s1.txt`). Items 1 and 2 hold from T1/T2 and B3, item 4 from B4, item 5 from every board run's stamps, and B2 has passed on the rebuilt image (D72). The line to write in `r/s1-runs.md` is therefore **"S1-F met (QNX plus Linux)"**, citing that parse file. The first wording was written under OD1 and **withdrawn** on the morning of 2026-09-17 when OD9 reversed the guest set; it is not the record. What this met line does and does not carry: item 3 is **completion only** and no figure from it is published (NC QDL v7 4.6(i)); the two-guest rung says nothing about duration (L6 `n/a`; the ten-minute evidence is B4's, with one guest); guest RAM's window stays unknown (Q14); the canaries bracket the rung and were not read while both guests ran; and it is no isolation, containment or freedom-from-interference claim. As §5.5 says, S1-F hands the freeze facts and decides none of its items: freeze item 2's fact is now "B5 ran and passed".
...
>
> **2026-09-17, later (OD9): B5 runs.** The owner reopened freeze item 2 and reversed it to **QNX plus Linux** — a genuine dual partition is preferred where it is feasible, and B2's `S1 ALLOC mib=1536 fill=ok verify=ok` settled feasibility. Pass item 3 is applicable again and this section is live design, not a record of a road not taken. **D14's limit is now derived: `--q2-limit 0x8E000000`**, with the derivation in the generator's geometry gate as §6.1 step 7 requires. What has *not* changed: ~~B5 has never run; R25 (two qvm instances on four cores) is UNKNOWN with no precedent in this project;~~ (**both superseded the same day — see the append below**) and under OD7 the guest disk is regenerated at the freeze, so an `s1-q2` built on today's `PIN_DISK` answers R25 but is not v1's image.
>
> **2026-09-17, later still: B5 ran and passed.** `s1-q2` — the Linux guest and the unchanged M3 QNX guest under native qvm at EL2 on four cores — ran on the board and met **pass item 3**: the QNX guest's `STAMP banner`, the IPC client's completion lines and `S1 BOTH alive`, beside L5 for Linux, with the client returning rc 0 and recording no sentinel recovery. `<rec>/B5/parse-s1.txt`: `verdict=pass`, `item3=pass`, `item5=ok`, tiers L0-L5 and L7. In the same directory's board log: all three canaries `verify=ok` at `canary_start`, before either guest launched, and again at `canary_post`, after teardown; `md5_post ok` for image, initrd, configuration, guest and disk, against the OD7-regenerated values; `S1 FAIL_STATE none`; the image reset itself, L4T returned unaided and `nvbootctrl` was equal to the session's first reading. R25 is answered, by **one** observation. **What this does not show:** nothing about duration with two guests — B5 is not a hold rung and L6 is `n/a`; the ten-minute evidence is B4's, with **one** guest. No timing or latency claim: item 3 is completion only, and every figure stays unpublished under NC QDL v7 4.6(i). Not that either guest's RAM came from window 2 (`guestram=unknown`; Q14 stands). No isolation, containment or freedom-from-interference claim. No mid-run canary check: the two reads bracket the rung and neither was taken while both guests ran. One observation, not a series. The image was built on the OD7-regenerated guest, so it answers R25 but is still not v1's image.
...
| R25 | Two qvm instances on four cores let the QNX guest's banner and IPC complete | ~~UNKNOWN (no two-VM precedent)~~ **2026-09-17: shown once, in B5** (completion only; one observation, no duration, timing, provenance or isolation claim; §6.10) | B5 |
...
- Before the D86 confirmatory run: `D86_B2_CONFIRM=yes` pre-registered in `J-waivers.conf`; §16.6.1 registered before the stage; the harness gate implemented with its self-tests (the key absent refuses; the key with one clean prior reading admits once; the key with a non-clean prior reading refuses; a second use refuses; every pre-existing B2 and J6x refusal still fires). The J6x arm stays closed. **2026-09-17 (applied):** the gate and those self-tests are in the working tree. Two things found in review are fixed with them: §16.6.1's X-i block is now a **refusal** rather than owner discipline alone — `s1-n1`, `s1-n2` and `s1-q2` stop while any B2 reading on file is not clean, naming the attempt and its class and routing to the owner — and a later non-clean reading can no longer hide behind an earlier clean one in the B2 refusal, which now names every reading the rung holds. A record with no class, and one whose class is exactly `n/a`, stay "no reading" and block nothing, as at the B2 gate itself. **2026-09-17, later (dated append): a third defect, found after B5 ran and fixed the same day on the owner's decision.** That X-i block admitted `X-f-final` alone, so a reading correctly classed **X-f-c** — this section's own name for a confirmatory run that read clean, whose row says B3-B5 may resume — would have made `r4bad` non-empty and blocked B3-B5, narrower than both the gate's own comment and §16.6.1. B5 was not affected: B2-a3's record reads `X-f-final`, because `--confirmatory` reached the parser after that reading was taken. The filter now admits both clean classes, with self-tests for `X-f-c` at B3, B4 and B5 and one pinning the boundary — `X-f-provisional` is not a final reading, so it still stops the ladder and routes to the owner. The sibling gate at the D86 key keeps its strict `X-f-final` test on purpose: an `X-f-c` on file there means the confirmatory run has already been made, and admitting it would breach the bound of one. No recorded reading moved and no class was re-labelled (precedence 6).
