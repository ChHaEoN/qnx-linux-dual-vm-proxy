...
- **H-C. The a1.metal hang is the same NISV mechanism.** Symptom identity only, n=1. Settled by one ftrace-instrumented re-run there
  ...
  - **2026-09-19: STILL OPEN, and deliberately not closed by that day's run.** A matched pair ran on a fresh a1.metal: the shipped
    startup reproduced the hang (17 bytes, `FOUND GICv3 ITS`) and a startup rebuilt with `-fno-auto-inc-dec` booted to the guest
    banner (1301 bytes), same host, same session, same launch line, same `disk-qemu`. That shows the defect **and its removal** both
    reproduce on Cortex-A72 — but **no trace was taken**, so it still does not show by data that the *2026-07-29* hang was NISV.
    H-C needs an ftrace-instrumented run, exactly as written. See `logs/sample-boot/aws-a1-metal-kvm-fix-crossvendor.log` and
    findings.md's 2026-09-19 entry.
...
| 4 | 7 | Tegra234 / Cortex-A78AE / VHE-specific silicon quirk | ~~**WEAKENED (n=1 on the second host)**~~ **REFUTED (2026-09-19)** | — | identical symptom on a1.metal (Annapurna A72, non-VHE host); still symptom-only there, one run; a c7g.metal or ftrace-instrumented a1.metal run would finish this. **2026-09-19: refuted by a matched pair on a fresh a1.metal — shipped startup hangs (17 bytes, `FOUND GICv3 ITS`), rebuilt with `-fno-auto-inc-dec` boots to the guest banner (1301 bytes), same host, session and launch line. Both the defect and its removal reproduce on Cortex-A72. The ftrace run this row asks for was still NOT done, so H-C remains open.** |
