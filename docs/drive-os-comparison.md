...
| **Inter-VM IPC** | Shared memory + mailbox, sub-µs | First hardware-timed cross-partition round trip: a 64-byte frame from L4T to the guest's monitor over a real bridge under KVM, **p50 0.172 ms, p99 0.461 ms** (n=3000/arm, governor pinned, 2026-09-19). Since 2026-09-22 also through shared memory: one slot in QEMU's `ivshmem`, both ends polling, **p50 4.5 µs** to the same monitor. | **Partial** — real cross-partition round trips are timed on silicon, but both are **host↔guest, not VM↔VM**: there is no hypervisor in the path and Linux owns the other side. The first is **TCP over virtio-net**; the second is shared memory with **no notification path** — both ends spin, holding a core or a vCPU. Neither is the quantity DRIVE OS's figure names. No VM↔VM or mailbox figure exists at any architecture, and per OD10 no hypervisor-path figure ever will. |
...
through a polled shared-memory slot in QEMU's `ivshmem` — and they are reported
