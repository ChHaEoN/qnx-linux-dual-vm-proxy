...
| **Inter-VM IPC** | Shared memory + mailbox, sub-µs | First hardware-timed cross-partition round trip: a 64-byte frame from L4T to the guest's monitor over a real bridge under KVM, **p50 0.172 ms, p99 0.461 ms** (n=3000/arm, governor pinned, 2026-09-19). Since 2026-09-22 also through shared memory: one slot in QEMU's `ivshmem`, both ends polling, **p50 4.5 µs** to the same monitor; and the same slot notified — the guest kicked over a virtio console, answering through the `ivshmem` doorbell — **p50 125 µs**. | **Partial** — real cross-partition round trips are timed on silicon, but all are **host↔guest, not VM↔VM**: there is no hypervisor in the path and Linux owns the other side. The first is **TCP over virtio-net**; the second is shared memory with both ends spinning, holding a core or a vCPU; the third is shared memory notified by a doorbell **in one direction only** — QEMU 6.2 interrupts a guest by MSI-X alone, which nothing here enables in this guest, so the host reaches it through a virtio console. None is the quantity DRIVE OS's figure names. No VM↔VM or mailbox figure exists at any architecture, and per OD10 no hypervisor-path figure ever will. |
...
filled with a number that measures something else. The figures this project does
have are *host↔guest* round trips — over TCP and virtio-net, and (2026-09-22)
through a shared-memory slot in QEMU's `ivshmem`, polled and notified — and they are reported
on their own rows against no reference.
...
| Inter-VM IPC P50 (VM↔VM, shared memory) | < 10 µs | **none.** The hypervisor's shared-memory path (`qvm vdev shmem`, A1) was functional-only, TCG-only and never timed; its notify half was deliberately skipped. OD10 removed the last route to a hypervisor-path figure. A VM↔VM figure under KVM, with two guests on one `ivshmem` file, has not been built, and the host↔guest `ivshmem` figures below are not this quantity. |
...
| **Host↔guest shared memory under KVM, polled** (this project's own quantity; **no DRIVE OS counterpart**) | — | **p50 4.5 µs, p99 6.1 µs** (medians over k = 12 rounds of each round's p50 and p99), 2026-09-22. One 64-byte slot in QEMU's `ivshmem`, L4T → QNX guest monitor, both ends spinning. Against the same kind of slot between two host processes, crossing into the guest adds nothing measurable (paired −0.0 µs, band ±0.3 µs). No notification, no hypervisor, and a core and a vCPU held while waiting; see [the run record](../results/orin-native-port/20260922T-a6-orin-shm/results.md). |
| **Host↔guest shared memory under KVM, notified** (this project's own quantity; **no DRIVE OS counterpart**) | — | **p50 125 µs, p99 161 µs** (medians over k = 12 rounds), 2026-09-22: the same slot, the guest kicked awake over a virtio console and answering through the `ivshmem` doorbell, which KVM turns into an eventfd write in the kernel (checked before the run: 10 000 doorbells, 4 exits to userspace). Both ends sleep, though KVM ended 18% of the vCPU's halts in this arm by halt polling, and CPU cost was not measured. The doorbell runs guest→host only; host→guest is the console. See [the run record](../results/orin-native-port/20260922T-a6-orin-kick/results.md). |
