...
| **QHV host image hangs on the Orin under the distro QEMU 6.2.0 TCG** (2026-09-08): boots to file-system mount, then never returns from the first timeout-wait (`waitfor /dev/random`; with an rng presented, `io-sock` start instead). **Root-caused to QEMU, not the host** (2026-09-09): `v6.2.0`'s `hw/arm/virt.c` has no NS-EL2 virtual-timer IRQ wiring (added in 9.0, `1ec896fe7c`); a VHE hypervisor's timers are `CNTHV_*`. `target/arm` also gained `5709038aa8` (10.0). Not bisected between them. The plain EL1 IFS is unaffected (`CNTV`, INTID 27, always wired). | `scripts/orin/build-qemu-on-orin.sh` builds a tagged QEMU (v11.1.0) into its own prefix — the QHV host **and guest boot** with it (~63–78 s to guest banner). Distro 6.2.0 kept intact for every prior `qnx-safety-vm` result. `launch-qhv-on-orin-tcg.sh` now selects the binary explicitly and stamps it. | Low (done); the from-source build also gives a `--enable-kvm` QEMU for a cheap NISV re-check |
