...
> `br0`/virtio-net under TCG did not stall at all. **KVM-accelerated boot was
> blocked when this leg ran**, by the root-caused ARM/KVM limitation described
> below, so TCG is what these numbers were measured under.
>
> **2026-09-18: that block is gone.** An IFS carrying a `startup-qemu-virt` we
> rebuilt with `-fno-auto-inc-dec` boots under `-enable-kvm` on this board,
> where the SDP's shipped one still stops after `FOUND GICv3 ITS`
> (`logs/sample-boot/orin-kvm-*.log`). Three limits, all load-bearing: the
> rebuilt startup is **ours**, so this is not a QNX-supported configuration;
> that run **took no timing at all**; and the A2 numbers above are **not**
> re-run or re-timed by it, so they remain TCG measurements exactly as
> recorded.
>
...
> was planned, not a current plan. **No hardware-timed *hypervisor* number has
> been measured on any leg** — that is what "hardware-timed" means throughout
> this repo, and it stays deferred. It does not mean "nothing was ever timed on
> hardware": the 2026-09-18/19 runs under A6 did time a QNX KVM guest on the
> board's own cores. The 2026-09-18 KVM boot itself took no timing at all.
>
...
> The risk register below also carries later rows: the `a1.metal` repro, the
> BSP-source compile check and the QHV leg on the Orin.
>
> **2026-09-18:** the 2026-07-28 sentence above — KVM boot blocked, TCG the accepted interim transport — now holds only for the SDP's **shipped** `startup-qemu-virt`, which still dies after `FOUND GICv3 ITS` (17 bytes). An IFS carrying a startup rebuilt from board source written in-project (`orin-native/startup/qemu-virt/`) with `-fno-auto-inc-dec` boots under `-machine virt,gic-version=3 -cpu host -enable-kvm` on this board, reaching procnto, `Startup complete` and the guest banner — [orin-kvm-fix-q620-run1.log](../logs/sample-boot/orin-kvm-fix-q620-run1.log) and the other `orin-kvm-*` captures; test arm run twice with byte-identical output, same result on QEMU 6.2.0 and 11.1.0. This is our own startup — **not a QNX-supported configuration**. The *hardware-timed* KVM number stays open: no timing, latency or boot-time figure was measured. The QHV/EL2 nested-virt limitation is a separate issue and is unchanged.
...
(step 4). That is a real deviation from this goal. The plain Phase-1 image
itself booted unchanged under TCG
...
| Console | `aliases { serial0 = &tcu }`, `stdout-path = "serial0:115200n8"`; `tcu` is `nvidia,tegra234-tcu` over HSP **mailboxes** — no MMIO UART | Not drivable by a polled callout. **2026-09-11 note:** superseded on the board. M0's shim wrote the TCU mailbox by polling after Linux had gone, and the SPE consumed the words ([findings.md](findings.md) 2026-09-09). In M1, procnto printed through a polled TCU callout this port wrote (`callout_debug_tcu.S`), watched live on the J14 header ([findings.md](findings.md) 2026-09-10). NVIDIA (KevinFFF, Apr 2024): ttyTCU0 on the Orin Nano is backed by `uartc@c280000`; "we don’t suggest and support for this use case" of using it as a normal UART; the attempt by the user gave no TX signal **[vendor forum]** https://forums.developer.nvidia.com/t/enabling-ttytcu0-as-regular-uart-on-orin-nano/287340 . TCU muxing runs "in the Sensor Processing Engine (SPE) for Jetson Orin" **[vendor]** https://docs.nvidia.com/jetson/archives/r36.4.4/DeveloperGuide/AT/JetsonLinuxDevelopmentTools/TegraCombinedUART.html |
...
new native Linux client (step 6). The plain Phase-1 `output/ifs.bin` itself booted
unchanged under TCG. The CSV
