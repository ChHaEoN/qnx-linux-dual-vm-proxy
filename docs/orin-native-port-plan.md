...
**RAM black box** per K10: mirror every shim/`put_tcu` byte into the ramoops carveout at `0x2725F0000`.
...
✓; WDT/TCU register peek ✓; governor ✓. **Still needed:** the ramoops zone offsets (K10, contested) and the
...
| 7 | ramoops zone offsets (contested: repo capture says the sysfs params read empty; a lens reports concrete values) | CONTESTED | Black box addressable only by the "last 256 KiB + `dd if=/dev/mem`" hack |
...
Single engineer, working days, adapter in hand from M0 step 2.
...
- [ ] **11b. Re-read the ramoops zone offsets** — `cat /sys/module/ramoops/parameters/*` and
  `dmesg | grep -i ramoops` — and save the raw output. Two sources disagree (K10) and the black box's
